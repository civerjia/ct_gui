...
| `test_idle_all_concurrency.py` | **run after any RP2350 power-service change.** Every filament to IDLE in one batch (no HV); fails unless all settle within the time bounds. Guards the concurrent CC/ramp service (RP2350 `architecture.md` §4.5 #11) — a serial one froze 83 filaments at ~700 mA |
| `test_result_repr.py` | Result's readable repr, against recorded runs, live read-only calls and edge cases — no hardware needed |
